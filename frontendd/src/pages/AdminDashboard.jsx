...
const AdminDashboard = () => {
  ...
  // const [districttt, setDistricttt] = useState(dashboardData.admin.district)
  ...
  const fetchPDSDashboard = async (pdsId) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:5000/api/admin/pds-dashboard/${pdsId}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  const fetchGodownDashboard = async (godownId) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:5000/api/admin/godown-dashboard/${godownId}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
    ...
    <div className="containerr">
      <div className="dashboard_container">
        ...
        <p className="proTitle">
          ...
          <strong>
            Transparent Stock Management In PDS Using CODE-128 FIFO Scheme
          </strong>
        </p>
        ...
        <div className="topcontainer">
          ...
          {dashboardData.admin && (
            <div className="admin-details">
              ...
              <ul>
                <li>
                  ID:<strong> {dashboardData.admin.id || "N/A"}</strong>
                </li>
                <li>
                  Name:<strong> {dashboardData.admin.name || "N/A"}</strong>
                </li>
                <li>
                  District:
                  <strong> {dashboardData.admin.district || "N/A"}</strong>
                </li>
              </ul>
            </div>
          )}
              ...
              {dashboardData.godowns.length > 0 ? (
                dashboardData.godowns.map((godown) => (
                  <tr key={godown.id}>
                    ...
                    <td>{godown.name}</td>
                    <td>
                      <button onClick={() => handleGodownSelect(godown.id)}>
                        {" "}
                        {/* Fixed typo here */}
                        View Details
                      </button>
        ...
        {selectedPDS && (
          <div className="selected-pds-details">
            ...
            <ul>
              <li>
                <strong>ID:</strong> {selectedPDS.id || "N/A"}
              </li>
              <li>
                <strong>Incharge Name:</strong> {selectedPDS.name || "N/A"}
              </li>
              <li>
                <strong>PDS Location:</strong>{" "}
                {selectedPDS.pdslocation || "N/A"}
              </li>
              <li>
                <strong>Linked Godown:</strong>{" "}
                {selectedPDS.godownname || "Not Linked"}
              </li>
              <li>
                <strong>District:</strong> {selectedPDS.district || "N/A"}
              </li>
            </ul>
            <h4>Products</h4>
            <table className="product-table">
        ...
        {selectedGodown && (
          <div className="selected-godown-details">
            ...
            <ul>
              <li>
                <strong>ID:</strong> {selectedGodown.id || "N/A"}
              </li>
              <li>
                <strong>Incharge Name:</strong> {selectedGodown.name || "N/A"}
              </li>
              <li>
                <strong>Godown Name:</strong>{" "}
                {selectedGodown.godownname || "N/A"}
              </li>
              <li>
                <strong>District:</strong> {selectedGodown.district || "N/A"}
              </li>
            </ul>
            <h4>Products</h4>
            <table className="product-table">
              ...
              <tbody>
                {selectedGodown.products &&
                selectedGodown.products.length > 0 ? (
