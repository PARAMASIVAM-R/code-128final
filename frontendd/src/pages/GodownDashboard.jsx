...
const GodownDashboard = () => {
  ...
  useEffect(() => {
     if (!godownData?.godownName) return;
     
     const roomName = `godown_${godownData.godownName}`;
     socket.emit("join_room", roomName);
     
     const handlePdsUpdate = (data) => {
       setNotifications(prev => [...prev, data]);
     };
 
     socket.on("pds_update", handlePdsUpdate);
     return () => socket.off("pds_update", handlePdsUpdate);
   }, [godownData]);
  ...
  //           receivedCode:decodedDataList.barcode,
  //         }),
  //       }
  //     );
  ...
  const addProductToGodown = async (productData) => {
    ...
    const dataToSend = {
      ...
      goodsInCount: productData.totalCount,
      date: `${productData.year}-${productData.month}-${productData.date}`,
      receivedCode: decodedDataList[0].barcodes,
    };
  
    console.log("Sending data to backend:", dataToSend); // <-- Log here
  ...
  //   if (decodedDataList) {
  //     const productData = {
  //       product: decodedDataList.product,
  //       weight: decodedDataList.weight,
  //       totalCount: decodedDataList.totalCount,
  //       year: decodedDataList.year,
  //       month: decodedDataList.month,
  //       date: decodedDataList.date
  //     };
  
  //     // Calling addProductToGodown with productData
  //     addProductToGodown(productData);
  //   }
  // }, [decodedDataList]);
      ...
      const response = await fetch(
        "http://localhost:5000/api/godown/send-to-pds",
        {
          ...
          body: JSON.stringify({
            ...
            deliveredCode:decodedDataList[0].barcodes,
          }),
        }
      );
  ...
  useEffect(() => {
    if (decodedDataList.length > 0) {
      ...
      console.log('receivedCode: decodedDataList.barcodes :', decodedDataList[0].barcodes);
    ...
    <div className="containerr">
      ...
      <div className="row">
        ...
        <p className="proTitle" >
         <strong>Transparent Stock Management In PDS Using CODE-128 FIFO Scheme</strong>
      </p>
      <div className="topcontainer">
        <h2 className="dashboard">Godown Incharge Dashboard</h2>
...
{/* 
        ...
        <div className="scanned-products">
          <h4>Scanned <div className="notifications-section">
